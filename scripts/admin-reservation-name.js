(() => {
          ...
          editor.innerHTML = `
            <div class="admin-form-row admin-reservation-name-row">
              ...
              <div class="admin-reservation-name-controls">
                <input type="text" data-reservation-customer-name autocomplete="off" placeholder="실명 또는 확인한 이름" value="${escapeHtml(adminName)}" />
  ...
  list.addEventListener('click', async (event) => {
    ...
    const editor = button.closest('[data-reservation-name-editor]');
    const input = editor?.querySelector('[data-reservation-customer-name]');
    const id = String(card?.dataset?.reservationId || '');
    const client = window.barunjariAdmin?.client;
    if (!client || !id || !input) return;
  ...
  list.addEventListener('keydown', (event) => {
    ...
    input.closest('[data-reservation-name-editor]')?.querySelector('[data-save-reservation-name]')?.click();
  });

  const observer = new MutationObserver(() => window.requestAnimationFrame(enhanceCards));
